...
@Component({
  ...
  template: `
    <div
      class="graph-search border-1 border-primary-300 border-round-2xl surface-ground p-1"
      [style.max-height]="
        layout.minimal ? 'min(25vh, 18rem)' : 'min(50vh, 38rem)'
      "
    >
      <div
          ...
          <div *ngIf="layout.minimal; else full">
            <app-ks-message
              ...
              *ngIf="source.title.length > 0"
              ...
              (contextmenu)="
                onContextMenu.emit({ data: [source], event: $event })
              "
              [ks]="source"
            >
            </app-ks-message>
      ...
      <div
        class="w-full text-center pb-1"
        [class.pt-2]="sources && sources.length > 0"
      >
        <div
          *ngIf="sources && sources.length > 0; else footer"
          class="w-full flex-row-center-between px-4 text-600"
        >
          <div
...
export class GraphSearchComponent implements OnInit, OnDestroy, OnChanges {
  ...
  @Output() onResultClicked = new EventEmitter<
    KnowledgeSource & SearchResult
  >();
  ...
  ngOnInit(): void {
    try {
      const minimal =
        JSON.parse(localStorage.getItem('graph-search-minimal') ?? '') ?? false;
      this.layout =
        this.layoutOptions.find((l) => l.minimal == minimal) ??
        this.layoutOptions[0];
      this.onLayoutChange();
    } catch (e) {
      this.layout =
        this.layoutOptions.find((l) => l.minimal) ?? this.layoutOptions[0];
      this.onLayoutChange();
    }
  }
  ...
  onLayoutChange() {
    localStorage.setItem(
      'graph-search-minimal',
      JSON.stringify(this.layout.minimal)
    );
    this.scroll();
  }
}
