...
@Component({
  ...
  template: `
    <div class="w-full h-full">
      ...
      <graph-search
        [sources]="(searchSources | async) ?? []"
        [selectedIndex]="selectedIndex"
        footerText="{{ projectNodeCount }} Projects, {{
          sourceNodeCount
...
export class GraphCanvasComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() onSourceTap = new EventEmitter<{
    data: KnowledgeSource;
    event: MouseEvent;
  }>();

  @Output() onSourceDblTap = new EventEmitter<{
    data: KnowledgeSource;
    event: MouseEvent;
  }>();

  @Output() onSourceCtxtap = new EventEmitter<{
    data: KnowledgeSource[];
    event: MouseEvent;
  }>();

  @Output() onProjectTap = new EventEmitter<{
    data: KcProject;
    event: MouseEvent;
  }>();

  @Output() onProjectDblTap = new EventEmitter<{
    data: KcProject;
    event: MouseEvent;
  }>();

  @Output() onProjectCtxtap = new EventEmitter<{
    data: KcProject;
    event: MouseEvent;
  }>();
  ...
  private _searchSources: BehaviorSubject<(KnowledgeSource & SearchResult)[]> =
    new BehaviorSubject<(KnowledgeSource & SearchResult)[]>([]);

  searchSources: Observable<(KnowledgeSource & SearchResult)[]> =
    this._searchSources.asObservable();
      ...
      .pipe(
        ...
        tap((graphSettings) => {
          this.commonOptions = {
            ...
            animationDuration: graphSettings.animation.enabled
              ? graphSettings.animation.duration
              : 0,
            fit: true,
            nodeDimensionsIncludeLabels: true,
            maxSimulationTime: graphSettings.simulation.infinite
              ? 9999999
              : graphSettings.simulation.maxTime,
      ...
      .pipe(
        ...
        tap((selected: any[]) => {
          ...
          const elems = this.cy
            ...
            .nodes('[type="ks"]')
            .select();
          const sources: (KnowledgeSource & SearchResult)[] =
            elems?.map((s) => s.data('ks')) ?? [];
          this.highlightPath();
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    prevData.forEach((p: any) => {
      ...
      if (
        currMatches.length === 1 &&
        currMatches[0].data.label !== p.data.label
      ) {
   ...
   */
  highlightPath(selector = ':selected', root = '[type="root"]') {
  ...
  onFitToView(timeoutms = 1000) {
    this.cy
      ?.animation({
        ...
        duration: this.settings.get().app.graph.animation.enabled
          ? timeoutms
          : 0,
  ...
  async onRun() {
    this.ngZone.runOutsideAngular(() => {
      ...
      if (
        this.settings.get().app.graph.simulation.enabled &&
        this.cyLayout.name !== 'cola'
      ) {
        const simLayout = this.graphLayouts.layouts.find(
          (l) => l.name === 'Simulate'
        )?.options;
        if (simLayout) {
          setTimeout(() => {
  ...
  onSearch(term: string) {
    ...
    this.cy
      ?.elements()
      .removeClass(
        'search-result not-search-result search-path-edge search-path-node'
      )
      .unselect();
    ...
    const results = this.search.graph(
      this.data.map((v) => v.data.ks),
      term
    );
   ...
   */
  async onCenter(selector: string, timeoutms = 1000, zoom = 1) {
    return this.cy
      ?.animation({
        zoom: zoom,
        center: {
  ...
  saveGraph() {
    ...
    const png64: Blob | undefined = this.cy?.png(options);
    if (png64) {
  ...
  private add(data?: any) {
    ...
    this.projectNodeCount =
      1 + (this.cy?.nodes('[type="project"]').length ?? 0);
  ...
  private setListeners() {
    ...
    const emit = (emitter: EventEmitter<any>, data: any, event: MouseEvent) =>
      emitter.emit({ data: data, event: event });
    ...
    this.ngZone.runOutsideAngular(() => {
        ...
        if (!shiftKey && !ctrlKey && !metaKey) {
          ...
          if (ks) {
            ...
            this.notifications.error(
              'Graph Canvas',
              'Invalid Source',
              'Event target should include a copy of the Source, but it does not.'
            );
          }
        }
      });
      ...
      this.cy.on('cxttap', 'node[type="ks"]', (event: any) => {
        // @ts-ignore
        const select: KnowledgeSource[] = this.cy
          ?.nodes(':selected') // @ts-ignore
          .map((n) => n._private.data.ks)
  ...
  private removeStyles() {
    this.cy
      ?.elements()
      .removeClass('search-path-node search-path-edge')
      .unselect();
  }
}
